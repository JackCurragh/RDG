...
def extract_translons(
    ...
    - starts (Set[str]): Set of start codons to initiate translon detection. Default: {"ATG", "CTG", "GTG"}.
    - min_length (int): Minimum length of translons to be included in the result. Default: 10.

    Returns:
    List[Tuple[int, int]]: A list of tuples representing the start and stop positions of detected translons.
            ...
            for j, stop_codon in enumerate(codon_list):
                if stop_codon in stops:
                    ...
                    if len(translon) > min_length:
                        ...
                        translons.append((start_codon_position, stop_codon_position))
                    break
...
def build_graphs_from_fasta(
    ...
    - min_length (int): Minimum length of translons to be included in the result. Default: 100.
    - start_codons (Set[str]): Set of start codons to initiate translon detection. Default: {"ATG", "CTG", "GTG"}.
    ...
    for graph in graphs:
        ...
        for translon in graph.open_reading_frames:
            print(f"translon start: {translon.start}, translon stop: {translon.stop}")
    ```
    """
